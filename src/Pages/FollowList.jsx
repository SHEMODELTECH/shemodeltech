    ...
    <div key={user.id} className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-xl">
      <Link to={`/profile/${encodeURIComponent(user.email)}`} className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0">
          <p className="text-gray-900 text-sm font-semibold truncate hover:text-pink-600">{user.displayName || 'User'}</p>
        ...
        {tabs.map(t => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-all ${
              activeTab === t.key ? 'border-pink-600 text-pink-600' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
            {t.label}
            <span className={`px-1.5 py-0.5 rounded-full text-xs ${activeTab === t.key ? 'bg-pink-100 text-pink-700' : 'bg-gray-100 text-gray-500'}`}>
              {t.count}
            </span>
          </button>
        ))}
