...
const LandingPage = () => {
 ...
 <p id="hero-desc" data-animate className={`fade-up delay-2 text-gray-600 text-lg sm:text-2xl max-w-3xl mx-auto mb-10 font-normal leading-relaxed ${isVisible('hero-desc') ? 'visible' : ''}`}>
 She Model Tech is where women build real tech experience. Join a team, ship a real project, and earn verified badges that show employers exactly what you did. Free, always.
 </p>
 ...
 <h2 className="text-3xl sm:text-4xl md:text-5xl font-800 text-gray-900 mb-4">What You Can Do with SHE MODEL TECH</h2>
 <p className="text-gray-500 max-w-xl mx-auto text-base">
 From real collaborative projects to verified credentials, She Model Tech gives you everything you need to build and prove your skills.
 </p>
 </div>

 <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
 {features.map((f, i) => (
 <div key={i} id={`feature-${i}`} data-animate className={`fade-up delay-${i + 1} border border-gray-200 rounded-2xl p-7 hover:border-pink-300 hover:shadow-sm transition-all ${isVisible(`feature-${i}`) ? 'visible' : ''}`}>
 ...
 {badges.map((b, i) => (
 <div key={i} id={`badge-${i}`} data-animate className={`fade-up delay-${i + 1} bg-white border border-gray-200 rounded-2xl p-5 text-center hover:border-pink-300 hover:shadow-sm transition-all ${isVisible(`badge-${i}`) ? 'visible' : ''}`}>
 ...
 <p className="text-pink-600 text-sm font-semibold uppercase tracking-widest mb-3">Simple Process</p>
 ...
 <p className="text-pink-600 text-sm font-semibold uppercase tracking-widest mb-3">For Companies</p>
 <h2 className="text-3xl sm:text-4xl md:text-5xl font-800 text-gray-900 mb-4">Hire proven talent</h2>
 <p className="text-gray-500 max-w-xl mx-auto text-base">
 She Model Tech is a community of builders verified by the work they have actually shipped. Hire from our talent pool and post paid projects, and judge every candidate on real, verifiable proof.
 </p>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
 <div id="company-0" data-animate className={`fade-up delay-1 border border-gray-200 rounded-2xl p-7 hover:border-pink-300 hover:shadow-sm transition-all ${isVisible('company-0') ? 'visible' : ''}`}>
 ...
 <div id="company-1" data-animate className={`fade-up delay-2 border border-gray-200 rounded-2xl p-7 hover:border-pink-300 hover:shadow-sm transition-all ${isVisible('company-1') ? 'visible' : ''}`}>
 ...
 "She Model Tech is everything we wished existed when we started our tech careers. One place to find projects, earn credentials, and connect with people who are building real things."
 </p>
 <div className="flex items-center justify-center gap-3">
 <div className="w-10 h-10 rounded-full bg-pink-600 flex items-center justify-center text-white font-bold text-sm">TA</div>
 ...
 <Link to="/terms" className="hover:text-pink-600 transition-colors font-medium">Terms</Link>
 <Link to="/privacy" className="hover:text-pink-600 transition-colors font-medium">Privacy</Link>
 <Link to="/support" className="hover:text-pink-600 transition-colors font-medium">Support</Link>
 </div>
