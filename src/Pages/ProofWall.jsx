...
const typeStyle = {
 ...
 lead: { bg: 'bg-pink-100', fg: 'text-pink-600', shape: 'rounded-xl', icon: 'M3 21v-4m0 0V5a2 2 0 012-2h6.5l1 2H21l-3 6 3 6h-8.5l-1-2H5a2 2 0 00-2 2zm9-13.5V9' },
...
const ProofWall = () => {
 ...
 <span key={m.uid} className="text-xs font-medium px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full flex items-center gap-1">
 @{m.name}
 <button onClick={() => setUpdateMentions(prev => prev.filter(x => x.uid !== m.uid))} className="text-pink-400 hover:text-pink-700">×</button>
 </span>
 ))}
 </div>
 )}
 ...
 <button onClick={handlePostUpdate} disabled={posting || uploadingImg} className="bg-pink-600 hover:bg-pink-700 text-white text-sm font-semibold px-4 py-1.5 rounded-lg disabled:opacity-50">
 ...
 filter === f.id ? 'bg-pink-600 text-white shadow-sm' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
 }`}
 >
 {f.label}
 </button>
 ...
 loadingTalent ? (
 <div className="flex justify-center py-16"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-pink-600"></div></div>
 ) : talent.length === 0 ? (
 ...
 <div className="w-12 h-12 rounded-full bg-pink-100 text-pink-600 flex items-center justify-center font-bold flex-shrink-0">{(m.name || 'M').charAt(0).toUpperCase()}</div>
 )}
 ...
 {p.track && <span className="inline-block mt-1 text-xs font-medium px-2 py-0.5 bg-pink-50 text-pink-600 rounded-full">{p.track}</span>}
 </div>
 ...
 <p className="text-pink-700 text-xs font-bold uppercase tracking-wide">Open projects</p>
 <button onClick={() => setFilter('open')} className="text-pink-600 text-xs font-semibold hover:underline">View all →</button>
 </div>
 <div className="flex flex-col gap-2">
 {openProjects.slice(0, 2).map(p => (
 <button key={p.id} onClick={() => navigate(`/projects/${p.id}`)} className="flex items-center justify-between gap-3 bg-white border border-gray-200 rounded-lg px-3 py-2 text-left hover:border-pink-300 transition-all">
 ...
 <button onClick={() => setFilter('talent')} className="text-pink-600 text-xs font-semibold hover:underline">View all →</button>
 </div>
 <div className="flex flex-col gap-2">
 ...
 <div className="w-8 h-8 rounded-full bg-pink-100 text-pink-600 flex items-center justify-center text-xs font-bold flex-shrink-0">{(m.name || 'M').charAt(0).toUpperCase()}</div>
 )}
 <div className="min-w-0 flex-1">
 ...
 {m.email && (
 <button onClick={() => navigate(`/profile/${m.email}`)} className="flex-shrink-0 text-pink-600 text-xs font-semibold hover:underline">Profile →</button>
 )}
 </div>
 ))}
 </div>
 ...
 <p className="text-gray-500 text-sm mt-1">{myData?.isCompany ? 'Updates from across She Model Tech will show up here.' : 'Share a project update, and it shows up here as proof of your work.'}</p>
 </div>
 ) : (
 <div className="flex flex-col gap-2.5">
 {visibleItems.map(a => {
 ...
 <div className="w-10 h-10 rounded-full bg-pink-600 text-white flex items-center justify-center text-sm font-bold">{initial}</div>
 )}
 </button>
 );
 })() : (
 ...
 <button onClick={() => saveEdit(a)} className="bg-pink-600 hover:bg-pink-700 text-white text-xs font-semibold px-3 py-1.5 rounded-lg">Save</button>
 ...
 {a.mentions.map((m, i) => (
 <span key={m.uid || i} className="text-xs font-medium px-2 py-0.5 bg-pink-50 text-pink-600 rounded-full">@{m.name}</span>
 ))}
 </div>
 )}
