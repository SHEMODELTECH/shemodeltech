  ...
  const activate = async (companyId, months, claimId) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'notifications'), {
        userId: companyId, recipientId: companyId,
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10 sm:py-14">
      ...
      <p className="text-gray-500 text-sm mb-6">
        Confirm payments and switch company access on. Access is granted here and
        nowhere else &mdash; never by landing on a redirect URL.
      </p>
      ...
      {!MEMBERSHIP_ENFORCED && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8">
          ...
          <p className="text-amber-800 text-xs leading-relaxed">
            Every company currently has full access for free. Nothing here is
            enforced yet. Set <code className="font-mono">MEMBERSHIP_ENFORCED = true</code> in{' '}
            <code className="font-mono">src/config/membership.js</code> when you&rsquo;re
            ready to charge &mdash; activations you grant now will already be in place.
          </p>
        </div>
      )}
      ...
      {claims.length === 0 && (
        <p className="text-gray-500 text-sm mb-8">No claims waiting.</p>
      )}
      ...
      <div className="space-y-2 mb-10">
        {companies.filter(c => c.tier !== 'partner').map(c => (
          <div key={c.uid} className="flex items-center justify-between gap-3 bg-white border border-gray-200 rounded-lg p-3 flex-wrap">
            ...
            <div className="flex gap-2">
              <button onClick={() => activate(c.uid, 1, null)} disabled={busy === c.uid}
                className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-gray-300 hover:bg-gray-50">
                1 month
              </button>
              <button onClick={() => activate(c.uid, 12, null)} disabled={busy === c.uid}
      ...
      <div className="space-y-2">
        {partners.map(c => (
          <div key={c.uid} className="flex items-center justify-between gap-3 bg-green-50 border border-green-200 rounded-lg p-3 flex-wrap">
            <div className="min-w-0">
              <p className="font-semibold text-gray-900 text-sm">
                {c.companyName || c.displayName || c.email}
              </p>
            ...
            <button onClick={() => revoke(c.uid)} disabled={busy === c.uid}
