...
const ActivateAccess = () => {
  ...
  useEffect(() => {
    if (!currentUser) {
      navigate('/login?next=/partner/activate');
      return;
    }
    let dead = false;
    ...
    (async () => {
      try {
        ...
        const claims = await getDocs(
          query(collection(db, 'activation_claims'), where('companyId', '==', currentUser.uid))
        );
        if (dead) return;
  ...
  if (existing) {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16 text-center">
        ...
        <Link
          to={activated ? '/talent-board' : '/dashboard'}
          className="inline-block bg-pink-600 hover:bg-pink-700 text-white font-semibold text-sm px-6 py-3 rounded-lg"
        >
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-14">
      ...
      <p className="text-gray-600 text-sm mb-8 leading-relaxed">
        Thanks for subscribing. Enter the reference from your receipt and we&rsquo;ll activate your
        access. We confirm every payment by hand, so this takes a little longer than instant, but it
        means nobody can get in without paying.
      </p>

      <label className="block text-sm font-bold text-gray-900 mb-1.5">Payment reference *</label>
      <input
        value={reference}
      ...
      <p className="text-gray-400 text-xs mt-6">
        Problems? Email{' '}
        <a href={`mailto:${BRAND.supportEmail}`} className="text-pink-600">
          {BRAND.supportEmail}
        </a>{' '}
        with your reference.
      </p>
