...
const HostCohort = () => {
  ...
  const totalBudget = roles.reduce(
    (sum, r) => sum + (Number(r.payAmount) || 0) * (parseInt(r.count, 10) || 1), 0
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10 sm:py-14">
      ...
      <p className="text-gray-600 text-sm mb-8 leading-relaxed">
        You own this one &mdash; the brief, the team, the timeline, and the work.
        Every applicant has already earned a verified badge building something
        real with us. Note that company projects don&rsquo;t award badges, since
        we don&rsquo;t review the work; members receive a paid work-experience
        record instead.
      </p>
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        <div>
          <label className="block text-sm font-bold text-gray-900 mb-1.5">Target completion *</label>
      ...
      {roles.map((r, i) => (
        <div key={i} className="bg-gray-50 border border-gray-200 rounded-xl p-3 mb-2">
          <div className="flex gap-2 mb-2">
            <input value={r.title} onChange={e => setRole(i, 'title', e.target.value)}
          ...
          <div className="flex gap-2">
            <input type="number" min="1" value={r.payAmount}
            ...
            <input value={r.skills} onChange={e => setRole(i, 'skills', e.target.value)}
              placeholder="Skills (optional)"
              className="flex-1 px-3 py-2 rounded-lg border border-gray-300 text-sm outline-none focus:border-pink-500" />
            {roles.length > 1 && (
              <button type="button" onClick={() => setRoles(rs => rs.filter((_, idx) => idx !== i))}
                className="text-gray-400 hover:text-red-600 text-xs px-2">Remove</button>
            )}
          </div>
        </div>
      ))}
      <button type="button" onClick={() => setRoles(rs => [...rs, blankRole()])}
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
        ...
        <p className="text-gray-600 text-xs leading-relaxed">
          You pay members directly &mdash; She Model Tech never holds the funds.
          When you mark the project complete, each member confirms she was paid.
          Unresolved non-payment is reviewed by our team and can end hosting
          access and your plan.
        </p>
      </div>
