  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 sm:py-16">
      ...
      <p className="text-gray-600 mb-12 max-w-2xl leading-relaxed">
        Every graduate has built a real product on a real team. Not a course
        certificate &mdash; a verified badge backed by commit history you can
        inspect, with a named role and a finished project behind it.
      </p>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-8">
        ...
        <p className="text-gray-400 text-xs mt-3">
          No card needed. Verification is free too &mdash; submit your registration
          details and members can see who they&rsquo;re dealing with.
        </p>
      </div>

      <div className="grid md:grid-cols-2 gap-5 mb-12">
        {/* Talent Access */}
        <div className="bg-white border-2 border-pink-500 rounded-2xl p-6">
          <p className="text-pink-600 font-bold text-xs uppercase tracking-wide mb-2">
            Recurring
          </p>
          <h2 className="text-xl font-bold text-gray-900 mb-1">Talent Access</h2>
          <p className="text-3xl font-bold text-gray-900 mb-1">
            {formatPrice(PLANS.TALENT_ACCESS_MONTHLY.amount, 'month')}
          </p>
          <p className="text-gray-400 text-xs mb-5">
            or {formatPrice(PLANS.TALENT_ACCESS_ANNUAL.amount, 'year')} &mdash; two months free
          </p>
          <ul className="text-gray-600 text-sm space-y-2 mb-6">
            {PLANS.TALENT_ACCESS_MONTHLY.features.map((f, i) => <li key={i}>{f}</li>)}
          </ul>
          <button
            onClick={() => checkout({
              kind: 'subscription',
              priceId: PLANS.TALENT_ACCESS_MONTHLY.priceId,
            }, 'sub')}
            disabled={busy === 'sub'}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6">
          <p className="text-purple-600 font-bold text-xs uppercase tracking-wide mb-2">
            One-off
          </p>
          <h2 className="text-xl font-bold text-gray-900 mb-1">Sponsor a team</h2>
          <p className="text-3xl font-bold text-gray-900 mb-1">
            {formatPrice(SPONSORSHIP_TIERS[0].amount)}
          </p>
          <p className="text-gray-400 text-xs mb-5">Funds stipends for ~5 women</p>
          ...
          <p className="text-gray-400 text-[11px] mb-5 leading-relaxed">
            Sponsors watch, acknowledge and recruit. The team owns its own brief,
            decisions and code &mdash; that&rsquo;s what makes the training real.
          </p>
          <button
      ...
      <p className="text-gray-500 text-sm text-center">
        Questions? <a href={`mailto:${BRAND.supportEmail}`} className="text-pink-600 font-semibold hover:underline">
          {BRAND.supportEmail}
        </a>
      </p>
    </div>
