...
const Sponsor = () => {
  ...
  const split = computeStipends(
    tier.amount,
    tier.id === 'sponsor_team' ? 5 : tier.id === 'sponsor_two_teams' ? 10 : 30
  );
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12 sm:py-16">
      ...
      <p className="text-gray-600 mb-8 leading-relaxed">
        Your sponsorship pays training stipends to a team of women building a real product over
        eight weeks. They finish with verified badges, a certificate, and something they can show an
        employer.
      </p>
        ...
        <>
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
            ...
            <p className="text-gray-600 text-xs leading-relaxed">
              <strong>{formatPrice(split.stipendPool)}</strong> goes directly to the women as
              training stipends, about {formatPrice(split.perMember)} each. The remaining{' '}
              {Math.round(OVERHEAD_RATE * 100)}% covers running the programme: mentoring, review,
              hosting and support.
            </p>
          </div>
        </>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 mb-8">
        ...
        <ul className="text-gray-500 text-sm space-y-1.5 mb-3">
          {SPONSOR_LIMITS.map((l, i) => (
            <li key={i}>{l}</li>
          ))}
        </ul>
        <p className="text-gray-500 text-xs leading-relaxed">
          The team owns its brief, its decisions and its code, that&rsquo;s what makes the training
          real, and it&rsquo;s what your name is attached to. If you need work built to your
          specification, host your own paid project instead: you&rsquo;d own that outright.
        </p>
      </div>
      ...
      {!paidLive ? (
        <div className="bg-pink-50 border border-pink-200 rounded-xl p-5 text-center">
          ...
          <a
            href={`mailto:${BRAND.supportEmail}?subject=Sponsoring%20a%20She%20Model%20Tech%20team`}
