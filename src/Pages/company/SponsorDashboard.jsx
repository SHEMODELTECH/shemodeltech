...
const STATUS_COPY = {
  ...
  [SPONSORSHIP_STATUS.DISTRIBUTED]: { label: 'Stipends paid', style: 'bg-pink-100 text-pink-700' },
    ...
    <div className="max-w-3xl mx-auto px-4 py-10 sm:py-14">
          ...
          return (
            <div key={s.id} className="bg-white border border-gray-200 rounded-xl p-5">
              <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
                ...
                <span
                  className={`text-[10px] font-bold px-2.5 py-1 rounded-full uppercase ${status.style}`}
                >
                  {status.label}
                </span>
              </div>
                ...
                <>
                  ...
                  {split && (
                    <p className="text-gray-500 text-xs mb-3">
                      {formatMoney(split.perMember)} stipend per member &middot;{' '}
                      {formatMoney(split.stipendPool)} to the team &middot;{' '}
                      {formatMoney(split.overhead)} programme costs
                    </p>
                  )}
              ...
              {report?.completed && (
                <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <p className="text-gray-700 text-xs leading-relaxed">
                    {report.womenSupported} women completed &ldquo;{report.project}&rdquo; and
                    earned {report.badgesEarned} verified badge
                    {report.badgesEarned === 1 ? '' : 's'} between {report.period}.{' '}
                    {formatMoney(report.totalToMembers)} went directly to members as training
                    stipends.
                  </p>
                </div>
              )}
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-5">
        ...
        <p className="text-gray-600 text-xs mb-3 leading-relaxed">
          You fund the programme and are acknowledged for it. You&rsquo;ll see progress, meet the
          team at demo day, and get a two-week first look at the graduates you funded, first look,
          never exclusivity.
        </p>
        ...
        <p className="text-gray-500 text-xs mt-3 leading-relaxed">
          If you need work built to your specification, that&rsquo;s a different arrangement, host
          your own paid project instead.
        </p>
      </div>
    </div>
  );
