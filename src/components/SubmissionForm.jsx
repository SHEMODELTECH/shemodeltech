...
const SubmissionForm = ({ project, onSubmit, submitting, disabled, disabledReason }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 sm:p-6">
      ...
      />
      {repoUrl && !parsed.valid && <p className="text-red-600 text-xs mt-1.5">{parsed.error}</p>}
      {parsed.valid && (
      ...
      <div className="mt-5 bg-pink-50 border border-pink-200 rounded-xl p-4">
        ...
        <p className="text-gray-600 text-xs mb-3 leading-relaxed">
          Your badge certifies what <em>you</em> built. Reviewers need access to the commit history
          to verify each person&rsquo;s contribution, a public link alone can&rsquo;t show who wrote
          what.
        </p>
        <ol className="text-gray-700 text-xs space-y-1 mb-3 list-decimal list-inside">
          {COLLABORATOR_STEPS.map((step, i) => (
            <li key={i}>{step}</li>
          ))}
        </ol>
      ...
      {errors.length > 0 && (
        <div className="mt-4 bg-red-50 border border-red-200 rounded-lg p-3">
          {errors.map((e, i) => (
            <p key={i} className="text-red-700 text-xs leading-relaxed">
              {e}
            </p>
          ))}
        </div>
