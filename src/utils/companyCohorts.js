...
export const canHostCohort = (company) => {
  ...
  if (!company.isVerified) {
    return {
      allowed: false,
      reason: 'Your company must be verified before hosting. Verification is free — submit your registration details.',
    };
  }
...
export const canApplyToCompanyCohort = async (uid) => {
  try {
    const snap = await getDocs(query(
      collection(db, 'member_badges'), where('memberUid', '==', uid)
    ));
    if (snap.empty) {
      return {
        allowed: false,
...
export const createCompanyCohort = async ({
  company, title, description, roles, startDate, endDate, notes,
}) => {
  ...
  if (unpaid.length) {
    throw new Error('Every role must carry a pay amount. Unpaid company work is not permitted on She Model Tech.');
  }
...
export const applyToCompanyCohort = async ({ cohortId, applicant, roleTitle, coverNote }) => {
  ...
  const dupes = await getDocs(query(
...
export const decideApplication = async ({
  applicationId, decision, payAmount, roleTitle, message, interviewAt, meetLink,
}) => {
  ...
  await notify(app.applicantUid, {
    ...
    title: decision === 'approved' ? 'You\u2019re on the team'
      : decision === 'interview' ? 'A company wants to interview you'
...
export const reportParty = async ({ cohortId, reporter, against, reason }) => {
  ...
  await updateDoc(doc(db, 'company_cohorts', cohortId), {
    reports: arrayUnion({
      by: reporter.email, against, reason: reason.trim(), at: new Date().toISOString(),
    }),
  });
};
...
export const getOpenCompanyCohorts = async () => {
  const snap = await getDocs(query(
    collection(db, 'company_cohorts'),
    where('status', '==', COMPANY_COHORT_STATUS.HIRING),
  ));
...
export const getMyWorkRecords = async (uid) => {
  const snap = await getDocs(query(
    collection(db, 'work_records'), where('memberUid', '==', uid)
  ));
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
};
...
const notify = async (uid, payload) => {
  ...
  try {
    await addDoc(collection(db, 'notifications'), {
      userId: uid, recipientId: uid, isRead: false, read: false,
