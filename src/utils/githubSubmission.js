...
export const parseRepoUrl = (raw) => {
  ...
  if (/gist\.github\.com/i.test(value)) {
    return {
      valid: false,
      error: 'That is a Gist, not a repository. Please link the project repository.',
    };
  }
  if (!/github\.com/i.test(value)) {
    return {
      valid: false,
      error: 'That does not look like a GitHub link. It should start with https://github.com/',
    };
  }
