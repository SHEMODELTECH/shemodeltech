...
export const applyToLead = async ({
  cohortId, applicant, rankedProjectIds, pitch, experience, availabilityHours,
}) => {
  ...
  if (!pitch || pitch.trim().length < 40) {
    throw new Error('Tell us a little more about why you want to lead — a couple of sentences at least.');
  }
  ...
  const live = existing.docs.find(d =>
    ![LEAD_APP_STATUS.WITHDRAWN, LEAD_APP_STATUS.REJECTED].includes(d.data().status)
  );
  if (live) {
...
export const rejectAsLeadInviteAsContributor = async ({
  appId, applicant, suggestedProjectId, suggestedRole, message, reviewer,
}) => {
  await updateDoc(doc(db, 'lead_applications', appId), {
  ...
  if (suggestedProjectId) {
    ...
    } catch (_) { /* fall back to generic wording */ }
  }
...
export const rejectApplication = async ({ appId, applicant, reason, reviewer }) => {
  ...
  await notify(applicant.applicantUid, {
    ...
    body: reason || 'You weren\u2019t selected to lead this cohort, but you can apply again next cycle.',
    link: '/cohort',
  });
};
...
export const getFallbackCandidates = (applications, unassignedProjectId) =>
  applications
    .filter(a => a.status === LEAD_APP_STATUS.SUBMITTED
              || a.status === LEAD_APP_STATUS.INTERVIEW_SCHEDULED)
