...
const scoreProject = (project, profile) => {
  ...
  const userSkills = (
    profile.skills && profile.skills.length
      ? profile.skills
      : norm(profile.specialization).split(',')
  )
    .map(norm)
    .filter(Boolean);
  ...
  if (projTrack && userTrack && projTrack === userTrack) {
    score += 30;
    reasons.push('Matches your track');
  } else if (projTrack && userInterests.includes(projTrack)) {
    score += 18;
  ...
  (project.proposedRoles || project.teamRoles || []).forEach((r) => {
    norm(r.skills)
      .split(',')
      .forEach((s) => {
        if (s.trim()) roleSkills.push(s.trim());
      });
  });
  let overlap = 0;
  userSkills.forEach((us) => {
    if (roleSkills.some((rs) => rs.includes(us) || us.includes(rs))) overlap++;
  });
  if (overlap > 0) {
    score += Math.min(overlap * 12, 36);
    reasons.push(`${overlap} skill${overlap > 1 ? 's' : ''} in common`);
  }
  ...
  const rankedTracks = suggestTracks(userText)
    .filter((t) => t._score > 0)
    .map((t) => t.id);
  if (rankedTracks.length) {
    ...
    if (projTrackIds.has(topTrack)) {
      score += 20;
      reasons.push('Fits your strongest track');
    } else if (rankedTracks.some((t) => projTrackIds.has(t))) {
      score += 10;
      reasons.push('Fits one of your tracks');
    }
  }
  ...
  if (hasOpenLevel) {
    score += 15;
    if (userLevel <= 1) reasons.push('Open to newcomers');
  }
  ...
  const hasLeadActive =
    project.status === 'active' && (project.leadConfirmed || project.submitterId);
  if (isLeadTrack && needsLead) {
    score += 25;
    reasons.push('Needs a lead - step up');
  } else if (!isLeadTrack && hasLeadActive) {
    score += 20;
    reasons.push('Open to join');
  } else if (!isLeadTrack && needsLead) {
    score += 5;
  } // still shippable, lower priority
...
export const matchProjects = async (profile, max = 6) => {
  ...
  scored.sort(
    (a, b) => b._score - a._score || (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)
  );
